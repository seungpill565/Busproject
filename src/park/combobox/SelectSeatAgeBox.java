...
public class SelectSeatAgeBox extends JComboBox<String>{ // 연령별 좌석개수를 고를 콤보박스
	...
	public SelectSeatAgeBox(ReservationInfo user, int seatCnt) { // 예매정보에 연령 좌석 추가해야해서 클래스 그대로 이끌어오고 어떤 좌석에 대한 콤보박스인지 확인해야해서 배열 인덱스로 쓸 seatnum 데려온다
		...
		addItem(String.format("%d번 좌석", user.getSeatKeys().get(seatCnt))); // i번째 키 값에 해당하는 좌석 번호를 가장 위에 표시
		addItem("------");
	...
	void selectSeat() {
		addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(age.equals("------")||age.equals(String.format("%d번 좌석", user.getSeatKeys().get(seatCnt)))) {// 키 값을 고르거나 ---- 고르면 다시 선택하게
					new JOptionPane().showMessageDialog(null, "성인, 청소년, 아동 중 선택하세요.","오류",JOptionPane.ERROR_MESSAGE); // 성인 어린이 청소년 제외 선택하면 오류창 뜨게
				}else {
					discount(age);
					user.getSeatName().put(user.getSeatKeys().get(seatCnt), discountRate); // 해당 좌석번호 자리에 할인율 넣는다
					System.out.println(user.getSeatName().get(user.getSeatKeys().get(seatCnt))+" 저장 완료 " ); // 해쉬맵에 저장되는지 확인
				}
			}
		});
	}
